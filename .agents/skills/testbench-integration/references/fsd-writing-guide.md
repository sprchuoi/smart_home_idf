...
Appendix chapters with testbench-specific content. The fsd-engineer FSD uses the
layer-grouped Parts scheme, so locate these by role/title (under Part E / back
matter), not by a fixed section number.

## Step 8: Write the Operational Procedures chapter (Testbench Operations)

Replace the **Operational Procedures** chapter (under Part E) with
testbench-specific operational content. This chapter becomes a standalone
**operations guide** -- how to interact with the device through the testbench. It
contains no test cases.

If the `define` skill left a generic Operational Procedures chapter, replace it
entirely. The testbench operations are the operational procedures for this project.

### 8a. Hardware setup

Query the testbench for hardware details:
```bash
curl -s $TESTBENCH_URL/api/devices | jq .
curl -s $TESTBENCH_URL/api/info | jq .
```
...
| What | Where |
|------|-------|
| ESP32 USB | Testbench slot <N>, serial at `rfc2217://<bench>:<PORT>` |
| Testbench host | `<bench>:8080` |
| UDP log sink | `<bench-ip>:5555` (dotted-quad — the DUT has no mDNS) |
| OTA firmware URL | `$TESTBENCH_URL/firmware/<project>/<project>.bin` |
...
| Testbench AP SSID | `WB-TestAP` |
| Testbench AP password | `wbtestpass` |
...
| Testbench AP SSID | The WiFi network the testbench creates for the device to join | Passed in `enter-portal` request |
| Testbench AP password | Password for the testbench's AP | Passed in `enter-portal` request |
...
1. Upload firmware to the testbench (`/api/firmware/upload`)
2. Trigger OTA via BLE (`CMD_OTA` opcode) or via HTTP (`POST /ota` through relay)
3. Monitor result via serial

### 8f. HTTP endpoints

Document the device's HTTP endpoints and how to reach them via the testbench HTTP relay (`/api/wifi/http`). Typical endpoints: `/status`, `/ota`.
...
Replace the **Verification & Validation** chapter (under Part E) with testbench
...
replace the phase verification tables with testbench-specific test procedures.
...
|------|---------|---------------|-----------------|
| 1 | <feature> | <brief description, reference testbench chapter> | <expected output> |
```
...
testbench-specific diagnostics and reference material.

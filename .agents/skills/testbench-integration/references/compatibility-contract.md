...
The testbench is not a passive observer -- it actively drives the device through
...
The testbench skills detect device state by grepping serial and UDP log output
...
format string, the corresponding testbench skill will fail to detect the event.

The firmware must emit these exact strings at the specified locations:

| Pattern | Testbench skill that needs it | Where | Why the testbench needs it |
|---------|-------------------------------|-------|---------------------------|
| `"Init complete"` | serial monitor | End of app_main() | Confirms boot finished; testbench waits for this before proceeding |
| `"alive %lu"` | serial monitor | Heartbeat task | Proves device is running, not hung |
| `"OTA succeeded"` / `"OTA failed"` | OTA skill | OTA task | Confirms OTA result; testbench blocks until one appears |
...
| `"AP mode: SSID='%s'"` | WiFi skill | WiFi AP start | Testbench detects AP name to drive captive portal |
| `"Portal page requested"` | WiFi skill | portal_get_handler() | Confirms testbench HTTP request reached the portal |
...
| `"STA got IP"` | WiFi skill | wifi_event_handler() | Confirms device joined the network; testbench proceeds |
| `"STA disconnect, retry"` | WiFi skill | wifi_event_handler() | Diagnoses WiFi connection failures |
| `"BLE NUS initialized"` | BLE skill | BLE init | Confirms BLE is ready; testbench can start scanning |

Step 5 of the procedure ensures every required pattern exists in the firmware.

## Contract 2: Required Process Flows

The testbench automates device operations by driving specific sequences. The
...
SmartConfig instead of SoftAP) are not compatible with the testbench.

### WiFi Provisioning Flow (Captive Portal)

The testbench drives this exact sequence:
...
2. Testbench connects to device's SoftAP (via enter-portal)
   -> testbench sends HTTP GET to portal page
   -> firmware logs "Portal page requested"
   -> testbench submits credentials via POST
...
```
1. Testbench sends CMD_WIFI_RESET via BLE NUS
...
```
1. Testbench uploads firmware binary to its HTTP server
2. Testbench triggers OTA via one of:
   a. BLE: sends CMD_OTA via NUS -> firmware logs "OTA update requested"
   b. HTTP relay: POST to device's /ota endpoint
3. Device downloads firmware from testbench URL
...
```
1. Testbench scans for device by BLE name
2. Testbench connects to device's NUS service
3. Testbench writes binary commands to NUS RX characteristic:
...
message (so the testbench knows init is done).
...
|----------|-----------|------------|
| FSD specifies BLE provisioning instead of captive portal | FR mentions "BLE provisioning" or "SmartConfig" | Flag to user: testbench requires captive portal flow |
| FSD specifies MQTT-based OTA instead of HTTP | FR mentions MQTT OTA trigger | Flag to user: testbench requires HTTP `/ota` endpoint |
| FSD specifies custom BLE protocol instead of NUS | Interface spec shows non-NUS UUIDs | Flag to user: testbench requires NUS for BLE commands |
